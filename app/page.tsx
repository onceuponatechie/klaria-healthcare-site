      ...
      <div className="overflow-hidden py-6 sm:py-8 mask-fade-x pause-on-hover">
        <div className="flex gap-10 sm:gap-14 ticker whitespace-nowrap">
          {row.map((t, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-3 text-base sm:text-lg font-display font-semibold tracking-tight text-foreground/55 hover:text-primary transition-colors"
            >
        ...
        {[
          {
            icon: Clock,
            t: "Zero wait times",
            b: "Book a slot, walk in, get seen.",
          },
          {
            icon: Stethoscope,
            t: "One care team",
            b: "Specialists collaborating, not siloed.",
          },
          {
            icon: Sparkles,
            t: "Modern technology",
            b: "Gentler tools. Better outcomes.",
          },
        ].map((it, i) => (
          <Reveal key={it.t} delay={i * 0.08}>
            <motion.div
              whileHover={{ y: -3 }}
              className="rounded-2xl bg-card p-5 shadow-soft flex items-center gap-4"
            >
        ...
        >
          ...
          <div className="absolute bottom-5 left-5 right-5 text-background space-y-1">
            <div className="flex gap-2 mb-2">
              <span className="rounded-full bg-background/20 backdrop-blur-md px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider">
                Calm
              </span>
              <span className="rounded-full bg-background/20 backdrop-blur-md px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider">
                Quiet
            ...
            <p className="text-xl sm:text-2xl font-bold tracking-tight leading-tight">
              Spaces designed to lower your pulse.
            </p>
            <p className="text-sm text-background/80">
              Soft lighting, real plants, no fluorescent buzz.
            </p>
          </div>
        </motion.div>
      </Reveal>
...
const CYCLE_STATS = [
  ...
  },
  {
    ...
    body: "From 1,200+ verified reviews on Google, Healthgrades, and Zocdoc.",
  },
  {
    value: 3,
    suffix: " min",
    title: "Average wait time",
    body: "Book a slot, walk in, get seen. We respect your time as much as ours.",
  },
  {
    value: 98,
    suffix: "%",
    title: "Would recommend us",
    body: "To a friend, a parent, a child. The truest measure of care done right.",
  },
];
...
function CyclingStatCard() {
  ...
  const prev = () =>
    setI((p) => (p - 1 + CYCLE_STATS.length) % CYCLE_STATS.length);
  return (
    <motion.div
      whileHover={{ y: -6 }}
      transition={{ type: "spring", stiffness: 250 }}
    ...
    >
      ...
      <div className="relative h-full flex flex-col justify-between gap-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="rounded-full bg-background/15 backdrop-blur-md px-3 py-1.5 text-xs font-semibold">
            This Year
          </span>
        </div>
        ...
        <div className="space-y-3 min-h-0">
          ...
          <div className="flex items-center justify-between pt-2">
            <button
              ...
              className="grid place-items-center w-9 h-9 rounded-full bg-background/15 backdrop-blur-md hover:bg-background/25 transition"
            >
            ...
            <div className="flex items-center gap-1.5 flex-1 mx-3">
              {CYCLE_STATS.map((_, idx) => (
                <span
                  key={idx}
                  className={`h-1 rounded-full transition-all ${idx === i ? "flex-[2] bg-background" : "flex-1 bg-background/30"}`}
                />
              ))}
            </div>
            <button
...
function AnimatedStat({ value, suffix }: { value: number; suffix: string }) {
  ...
  const display =
    value % 1 !== 0 ? n.toFixed(1) : Math.round(n).toLocaleString();
  return (
    <p className="font-display text-5xl sm:text-6xl font-extrabold tracking-tighter leading-none">
      {display}
      <span className="text-background/70">{suffix}</span>
    </p>
  );
}
  ...
  const items = [
    {
      img: sDent,
      eyebrow: "Dentistry",
      title: "Bright smiles, painless visits.",
      body: "Cosmetic, restorative, and preventive dentistry by specialists who actually explain things.",
      icon: HeartPulse,
    },
    {
      img: sOpt,
      eyebrow: "Optometry",
      title: "Sharper vision, in one sitting.",
      body: "Comprehensive eye exams, contact lens fitting, and same-day designer frames.",
      icon: Microscope,
    },
    {
      img: sPhy,
      ...
      body: "Personalized rehab and sports physio with measurable recovery milestones.",
      icon: ShieldCheck,
    },
  ];
  return (
    <section id="story" className="bg-surface/60 border-y border-border/60">
      <div className="max-w-6xl mx-auto px-5 sm:px-8 py-20 sm:py-28 space-y-20 sm:space-y-28">
        ...
        {items.map((it, i) => (
          <div
            key={it.title}
            className={`grid lg:grid-cols-12 gap-8 lg:gap-14 items-center ${i % 2 ? "lg:[&>*:first-child]:order-2" : ""}`}
          >
            ...
            >
              ...
              <h3 className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight text-balance">
                {it.title}
              </h3>
          ...
          {row.map((d, i) => (
            ...
            >
              ...
              <div className="absolute bottom-5 left-5 right-5 text-background">
                <p className="text-xs font-semibold tracking-widest uppercase text-background/80">
                  {d.role}
                </p>
                <p className="mt-1 text-2xl font-extrabold tracking-tight leading-tight">
                  {d.name}
                </p>
        ...
        <div className="flex gap-5 ticker-reverse w-max">
          {[...row].reverse().map((d, i) => (
            ...
            >
              ...
              <p className="absolute bottom-3 left-3 text-background text-sm font-semibold">
                {d.name.split(" ").slice(-1)[0]} · {d.role}
              </p>
            </motion.div>
          ))}
        </div>
          ...
          <div className="flex items-center gap-2 pt-4">
            ...
            <div className="ml-3 flex gap-2">
              <button
                ...
                className="grid place-items-center w-10 h-10 rounded-full bg-card shadow-soft text-foreground hover:bg-foreground hover:text-background transition"
              >
                <ChevronDown className="w-4 h-4 rotate-90" />
              </button>
              <button
  ...
  const small = [
    {
      ...
      date: "Apr 18, 2026",
      read: "4 min",
      title: "The 5-minute morning routine our doctors actually do.",
    },
    {
      ...
      date: "Apr 9, 2026",
      read: "6 min",
      title: "What painless dentistry really means in 2026.",
    },
    {
      slug: "screen-fatigue",
      img: blog3,
  ...
  return (
    <section className="max-w-6xl mx-auto px-5 sm:px-8 py-20 sm:py-28">
      <Reveal className="flex flex-wrap items-end justify-between gap-6 mb-10 sm:mb-14">
        ...
        <Link
          href="/blog"
          className="group inline-flex items-center gap-2 rounded-full bg-card px-5 py-2.5 text-sm font-semibold text-foreground shadow-soft hover:bg-foreground hover:text-background transition-colors"
        >
        ...
        <Link href="/blog">
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 text-background">
              ...
              <p className="mt-3 text-background/85 max-w-xl text-base sm:text-lg hidden sm:block">
                {featured.excerpt}
              </p>
            </div>
          </motion.article>
              ...
              >
                ...
                <div className="p-6 flex-1 flex flex-col gap-3">
                  ...
                  <span className="mt-auto inline-flex items-center gap-2 text-sm font-semibold text-primary">
                    Read article{" "}
                    <ArrowRight className="w-4 h-4 magnetic-arrow" />
                  </span>
                </div>
              </motion.article>
            </Link>
  ...
  const faqs = [
    {
      ...
      a: "A 10-minute warm welcome with our care concierge, a full intake with your specialist, and clear next steps before you leave. Most first visits last 45–60 minutes.",
    },
    {
      q: "Do you accept my insurance?",
      a: "We work with most major insurance providers including Aetna, Cigna, BlueShield, and United Health. We'll verify your coverage before any visit, free of charge.",
    },
    {
    ...
    },
    {
      ...
      a: "Absolutely. Our pediatrics team specializes in newborns through teens, and our space is designed to make kids feel at ease.",
    },
    {
      q: "Do you offer telehealth consultations?",
      a: "Yes. Most follow-ups and many initial consultations can be done over secure video. Same booking flow, just choose 'Virtual'.",
    },
  ];
                ...
                >
                  ...
                  >
                    ...
                    <motion.span
                      ...
                      className={`grid place-items-center w-9 h-9 rounded-full shrink-0 ${isOpen ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground"}`}
                    >
          ...
          <Reveal className="lg:col-span-6 space-y-5">
            ...
            <ul className="grid gap-2 text-sm text-primary-foreground/90">
              <li className="flex gap-2 items-center">
                <BadgeCheck className="w-4 h-4" /> Same-day confirmation
              </li>
              <li className="flex gap-2 items-center">
                <BadgeCheck className="w-4 h-4" /> 24h free cancellation
              </li>
              <li className="flex gap-2 items-center">
                <BadgeCheck className="w-4 h-4" /> Most insurance accepted
              </li>
            </ul>
          </Reveal>
